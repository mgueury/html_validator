...
function tidyOnline() {
  var sHtml = oTidyUtil.pref.getHtml();
...
TidyViewSource.prototype = {
  ...
  changeFrame: function() {
    ...
    // Call tidy_devtools
    updateHtmlReport(url, true);
  },

  /** __ updateDocList  ___________________________________________
   */
  updateDocList: function(docList) {
    // currentFrame is used in getHtml to get the current frame.
    var select = document.getElementById("select_frame");
    select.options.length = 0;
    for (i = 0; i < docList.length; i++) {
      var option = document.createElement("option");
      option.value = docList[i];
